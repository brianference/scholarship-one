# QA report — sequence pass

Generated: 2026-07-18T06:40:45.802Z
Catalog: 209
Failures: 0

## Log

- `[smoke] OK: catalog 209`
- `[smoke] OK: top-5 audit awards present`
- `[smoke] OK: https urls`
- `[smoke] OK: sequence feature files present`
- `[integration] OK: CA audit search`
- `[integration] OK: NY TAP/Excelsior`
- `[integration] OK: disability search`
- `[integration] OK: 98 fixed / 110 cycle deadlines`
- `[integration] OK: score breakdown base constant`
- `[simulations] OK: pass 1/3 100/100`
- `[simulations] OK: pass 2/3 100/100`
- `[simulations] OK: pass 3/3 100/100`
- `[simulations] OK: 300 runs 189ms`
- `[simulations] OK: empty unexpected 0`
- `[stress-load] OK: 10000 @ 0.369ms`
- `[stress-load] OK: pathological ok`
- `[edge] OK: 28 state-tagged`
- `[edge] OK: top-5 states each have ≥2 awards`
- `[edge] OK: pipeline filter`
- `[edge] OK: status set size 4`
- `[a11y] OK: multi-page layout + results wired`
- `[a11y] OK: skiplink`
- `[a11y] OK: onboard modal a11y`
- `[a11y] OK: ScrollToTop present`
- `[visual] OK: new UI surfaces styled`
- `[security] OK: resend from env`
- `[security] OK: html escape`
- `[security] OK: no hardcoded keys in digest`
- `[security] OK: openai env`
- `[regression] OK: "handicapped student track math" ok`
- `[regression] OK: "California need-based" ok`
- `[regression] OK: "Texas undergrad financial need" ok`
- `[regression] OK: "Florida Bright Futures" ok`
- `[regression] OK: "New York TAP" ok`
- `[regression] OK: "Illinois MAP" ok`
- `[regression] OK: "DACA student" ok`
- `[regression] OK: "nursing student" ok`
- `[reliability] OK: CI runs qa`
- `[reliability] OK: CI runs build`
- `[reliability] OK: package qa script`
- `[reliability] OK: storage keys unique (5)`

## Failures

_None_

## Covered

1 Smoke 2 Integration 3 Sims 100×3 4 Stress 5 Edge 6 A11y 7 Visual 8 Security 9 Regression 10 Reliability/CI
