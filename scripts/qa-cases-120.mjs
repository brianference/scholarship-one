...
test(28, 'Nav Tracker is /tracker', () => site.includes("to: '/tracker'"))
...
test(36, 'applyProfileSearch navigates (to option, default /matches)', () => {
  const start = ctx.indexOf('const applyProfileSearch')
  const block = start >= 0 ? ctx.slice(start, start + 1200) : ''
  return block.includes('navigate(options?.to')
})
...
test(98, 'Header search input min-height', () => {
  const i = css.indexOf('.header-search__input')
  return i >= 0 && css.slice(i, i + 220).includes('min-height')
})
...
test(115, 'TrackerPage uses TrackerBoard', () => read('src/pages/TrackerPage.tsx').includes('TrackerBoard'))
